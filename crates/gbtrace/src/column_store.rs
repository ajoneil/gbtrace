...
/// (PartitionedStore) implement this. All consumers — framebuffer
...
impl ColumnStore {
    ...
    /// Add multiple rows to the row count (used by batch decoders).
    pub fn add_rows(&mut self, count: usize) {
        self.len += count;
    }

    /// Get a mutable reference to a column by index.
    pub fn column_mut(&mut self, col: usize) -> &mut ColumnData {
